...
sub get_anno_from_DB {
    my $ac = shift;
    my @allgo;
    my $sql = "select * from panther where uniprot='$ac'";
    #print STDERR $sql;
    ...
    return unless @row;
    foreach my $go (@row[6..8]) {
        next if not defined $go;
        my @go1 = split(/;/,$go);
        map {push @allgo, "GO:".$_;} @go1;
    }
    return(\@allgo, \@row);
}
#
...
open(GO,">OGs.go");
open(TGO,">OGs.tgo");
open(ANN,">OGs.ann.xls");
print TGO join("\t", 'gene_id', 'id'),"\n";
print ANN join("\t", 'OG','id','uniprot','EnsemblGenome','panther','fam','subfam','mf','bp','cc','pro_class','pathway'),"\n";
foreach my $gene (@genes) {
    my $ac = $gene_to_ac->{$gene};
    my ($a,$b) = get_anno_from_DB($ac);
    next unless $b;
    print GO join("\t", $gene, join(",",@$a)),"\n";
    map {print TGO join("\t", $gene, $_),"\n"} @$a;
    print ANN join("\t",$gene, @$b),"\n";
}
